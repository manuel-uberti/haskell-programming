...
startsWithVowel s = if isVowel (head s) then True else False
...
countTheBeforeVowel s = go (words s) 0
  where
    go [] acc = acc
    go ws acc = if isThe (head ws) then if startsWithVowel (head (tail ws))
        then go (tail ws) (acc + 1) else go (tail ws) acc else go (tail ws) acc

-- 3
countVowels :: String -> Integer
countVowels s = foldr (\a b -> if isVowel a then b + 1 else b) 0 s
...
mkWord s = if length vowels > length consonants then Nothing else Just (Word' s)
  where
...
integerToNat x
  ...
  where
    go x' acc = if x' == 0 then acc else go (x' - 1) (Succ acc)
...
-- 5
catMaybes :: [ Maybe a ] -> [ a ]
catMaybes xs = foldr (\a b -> if isJust a then just a : b else b) [] xs
  where
    just (Just x) = x
...
containsNothing (x : xs) = if isNothing x then True else containsNothing xs

flipMaybe :: [ Maybe a ] -> Maybe [ a ]
flipMaybe [] = Nothing
flipMaybe xs = if containsNothing xs then Nothing else Just (catMaybes xs)
...
-- 1
lefts' :: [ Either a b ] -> [ a ]
lefts' xs = foldr (\a b -> if isLeft a then left' a : b else b) [] xs
  where
    left' (Left x) = x
...
rights' :: [ Either a b ] -> [ b ]
rights' xs = foldr (\a b -> if isRight a then right' a : b else b) [] xs
  where
    right' (Right x) = x
...
treeBuild n = unfold
    (\x -> if x == 0 then Nothing else Just ( x - 1, x - 1, x - 1 )) n
