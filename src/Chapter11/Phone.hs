...
uniqueChars xs = foldr (\a b -> if elem a b then b else a : b) [] xs

longestCharSeq :: [ String ] -> String
longestCharSeq (x : xs) = foldl (\a b -> if length a > length b then a else b) x
    xs
...
uniqueWords xs = foldr (\a b -> if elem a b then b else a : b) [] xs

longestWord :: [ [ String ] ] -> [ String ]
longestWord (x : xs) = foldl (\a b -> if length a > length b then a else b) x xs
