...
myOr_ :: [ Bool ] -> Bool
myOr_ [] = False
myOr_ (x : xs) = if x == True then True else myOr_ xs
...
myAny_ f [] = False
myAny_ f (x : xs) = if f x == True then True else myAny_ f xs
...
-- 6
myFilter' :: (a -> Bool) -> [ a ] -> [ a ]
myFilter' f xs = foldr (\a b -> if f a == True then a : b else b) [] xs
