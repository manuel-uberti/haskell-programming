...
prop_listOrdered =
  forAll genListInt (\xs -> (listOrdered $ sort xs) == True)
...
prop_plusAssociative =
  forAll genInt (\x y z -> plusAssociative x y z)

prop_plusCommutative :: Property
prop_plusCommutative =
  forAll genInt (\x y -> plusCommutative x y)  
...
prop_timesAssociative =
  forAll genInt (\x y z -> timesAssociative x y z)

prop_timesCommutative :: Property
prop_timesCommutative =
  forAll genInt (\x y -> timesCommutative x y)  
...
prop_powerAssociative =
  forAll genInt (\x y z -> powerAssociative x y z)

prop_powerCommutative :: Property
prop_powerCommutative =
  forAll genInt (\x y -> powerCommutative x y)
