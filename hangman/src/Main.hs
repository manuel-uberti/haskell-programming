...
fillInCharacter (Puzzle word filledInSoFar s failed) c = Puzzle word
    newFilledInSoFar (c : s) failed
  where
...
addToFailed (Puzzle word filledInSoFar s failed) c = Puzzle word filledInSoFar s
    (c : failed)
...
gameOver :: Puzzle -> IO ()
gameOver (Puzzle wordToGuess _ guessed failed) = if (length failed) > 6 then do
    putStrLn "You lose!"
    putStrLn $ "The word was: " ++ wordToGuess
    exitSuccess else return ()

gameWin :: Puzzle -> IO ()
gameWin (Puzzle _ filledInSoFar _ _) = if all isJust filledInSoFar then do
    putStrLn "You win!"
    exitSuccess else return ()
